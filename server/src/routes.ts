...
import { verifySignUp, authJwt } from "./middleware";  // Adjust path if needed
...
router.post("/user/watchlist/add/:symbol", [authJwt.verifyToken], userController.addToWatchlist);
router.post("/user/watchlist/remove/:symbol", [authJwt.verifyToken], userController.removeFromWatchlist);
...
router.post("/stocks/:symbol/buy", [authJwt.verifyToken], stocksController.buyStock);
router.post("/stocks/:symbol/sell", [authJwt.verifyToken], stocksController.sellStock);

// News routes
router.get("/news", newsController.getNews);
router.get("/news/:symbol", newsController.getNews);

export default router;  // ✅ Now using ES6 export
